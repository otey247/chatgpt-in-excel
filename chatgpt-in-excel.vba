' This is a set of functions that will allow you to communicate with the OpenAI API within an Excel cell
...
' Author: Jo Otey
' http://github.com/otey247/chatgpt-in-excel
...
Private Function GetChatGPTResponse(prompt As String, encodeString As Boolean) As String
    ...
    temperature = "0.5"
    maxTokens = 300
    systemContent = "Based on the information I provide about an application migrating from on-premises to Azure, specify which Azure services would be needed in the environment to migrate the application. Your response should be limited to only listing Azure services with each service seperated by commas, for instance: Logic Apps, Azure Functions, Azure Web App." ' Define the content for the system role
    ...
    requestPayload = "{""model"":""" & model & """,""messages"":[{""role"":""system"",""content"":""" & systemContent & """},{""role"":""user"",""content"":""" & prompt & """}],""temperature"":" & temperature & ",""max_tokens"":" & maxTokens & "}"
    ...
    Debug.Print "Text Response: " & responseText
    
    ' Access the "choices" array and its elements
    Dim contentValue As String
    contentValue = jsonResponse("choices")(1)("message")("content")
    Debug.Print "Content Value: " & contentValue
    If contentValue <> "" Then
        GetChatGPTResponse = contentValue
    Else
        GetChatGPTResponse = "Error extracting response."
    End If
